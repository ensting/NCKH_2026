...
def find_src_ip_col(df):
    candidates = [
        "Src IP", "Source IP", "src_ip", "src ip",
        "Flow Src IP", "SrcAddr"
    ]
    normalized = {str(c).replace("\ufeff", "").strip().lower(): c for c in df.columns}
    ...
    raise ValueError(f"Không tìm thấy cột Src IP. Các cột hiện có: {list(df.columns)}")


def find_dst_ip_col(df):
    candidates = [
        "Dst IP", "Destination IP", "dst_ip", "dst ip",
        "Flow Dst IP", "DstAddr"
    ]
    normalized = {str(c).replace('\ufeff', '').strip().lower(): c for c in df.columns}
    ...
    raise ValueError(f"Không tìm thấy cột Dst IP. Các cột hiện có: {list(df.columns)}")
